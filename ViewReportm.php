...
$database = "auditplus";
    ...
    if(!$con)
      ...
      $sql1 = "SELECT COUNT(dept) FROM responses where dept = 'Hospital' AND year(date) = 2023;";
      $data1 = mysqli_query($con, $sql1);
      $res1 = mysqli_fetch_array($data1);
      $HospitalCount = $res1[0];

      $sql2 = "SELECT COUNT(dept) FROM responses where dept = 'Manufacturing' AND year(date) = 2023;";
      $data2 = mysqli_query($con, $sql2);
      $res2 = mysqli_fetch_array($data2);
      $ManufacturingCount = $res2[0];

      $sql3 = "SELECT COUNT(dept) FROM responses where dept = 'Education' AND year(date) = 2023;";
      $data3 = mysqli_query($con, $sql3);
      $res3 = mysqli_fetch_array($data3);
      $EducationCount = $res3[0];

      $sql4 = "SELECT COUNT(dept) FROM responses where dept = 'Hospitality' AND year(date) = 2023;";
      $data4 = mysqli_query($con, $sql4);
      $res4 = mysqli_fetch_array($data4);
      $HospitalityCount = $res4[0];

      $sql5 = "SELECT COUNT(dept) FROM responses where dept = 'Restaurant' AND year(date) = 2023;";
...
<body>
  ...
  <h1>Audit Data Dashboard</h1>
    ...
    <script>
      var RawMaterial = '<?=$HospitalCount?>';
      var Documentation = '<?=$ManufacturingCount?>';
      var Quality = '<?=$EducationCount?>';
      var Storage_Packeging = '<?=$HospitalityCount?>';
      var Safety = '<?=$RestaurantCount?>';
      var xValues =["RawMaterial", "Documentation","Quality","Storage_Packeging","Safety"];
      var yValues = [RawMaterial, Documentation, Quality, Storage_Packeging, Safety];
